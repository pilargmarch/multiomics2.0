...
```{r, eval = FALSE}
hTFtarget <- read.table(file = "scripts/associations/TF-gene/TF-Target-information.hTFtarget.txt", sep = "\t")
...
hTFtarget.breast <- hTFtarget[grep("breast", hTFtarget$tissue), ] # keep only the associations experimentally validated on breast tissue

hTFtarget.unclear <- hTFtarget[grep("unclear", hTFtarget$tissue), ] # we expand with associations where tissue was unclear

hTFtarget <- rbind(hTFtarget.breast, hTFtarget.unclear)

length(unique(hTFtarget$TF)) # 424
...
missing.TFs <- setdiff(unique(hTFtarget$TF), TF.conversion$external_gene_name) # 4
write.table(missing.TFs, file = "scripts/associations/TF-gene/missing.TFs.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
```
...
```{r, eval = FALSE}
missing.TFs <- read.table(file = "scripts/associations/TF-gene/hgnc-symbol-check-TF.csv", sep = ",")
...
```{r, eval = FALSE}
length(unique(hTFtarget$target)) # 34982
...
missing.targets <- setdiff(unique(hTFtarget$target), target.conversion$external_gene_name) # 13094

write.table(missing.targets, file = "scripts/associations/TF-gene/missing.targets.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)

missing.targets <- read.table(file = "scripts/associations/TF-gene/hgnc-symbol-check-target.csv", sep = ",")
...
TF.target.associations <- cbind(TF.target.associations[, 2], TF.target.associations[, 1]) # change order because paintomics expects regulators in column 2 and targets in column 1 for some reason ??
...
length(unique(all.TFs.and.targets)) # 15237

TF.expression <- subset(rna.expression, V1 %in% TF.target.associations$TF) # matrix with 346 transcription factors
...
TF.DEGs <- subset(rna.DEGs, V1 %in% TF.target.associations$TF) # 25 DEGs are transcription factors
...
relevant.TF.associations <- subset(TF.target.associations, V2 %in% TF.DEGs$V1) # 19147 TF-gene associations are relevant per this criterion
